export const translations = {
  en: {
    ...
    contact: {
      ...
      errorHintShort:
        "Tip: In EmailJS → Account → Security, add this page’s origin to “Allowed domains” (for local dev include http://localhost:5173). Your template should reference {{from_name}}, {{from_email}}, and {{message}}.",
      configMissing:
        "This form is not configured yet. Please email us directly at min@sihoooo.cn.",
      close: "Close"
    }
  },
  zh: {
    ...
    contact: {
      ...
      error: "发送失败，请重试。",
      errorHintShort:
        "提示：在 EmailJS → Account → Security 中，把当前页面的来源（开发环境请加上 http://localhost:5173）加入允许的域名。邮件模板中需使用变量 {{from_name}}、{{from_email}}、{{message}}。",
      configMissing:
        "表单尚未配置发信服务，请直接发邮件至 min@sihoooo.cn。",
      close: "关闭"
    }
  }
};
